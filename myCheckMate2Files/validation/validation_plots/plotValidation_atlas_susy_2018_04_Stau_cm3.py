...
resultFolder = '../validation_results_cm3/atlas_susy_2018_04'
...
for slhaFile in glob.glob(slhaFolder+'/*.slha'):
    ...
    data = data[data['analysis'] == 'atlas_susy_2018_04']
    ilow = np.where(data['sr'] == 'SR-lowMass')[0]
    ihigh = np.where(data['sr'] == 'SR-highMass')[0]

    # data = data[data['analysis'] == 'atlas_conf_2019_018']
    # ilow = np.where(data['sr'] == 'sr_lowmass')[0]
    # ihigh = np.where(data['sr'] == 'sr_highmass')[0]
...
plt.savefig("atlas_susy_2018_04_StauEffs_cm3.png")
...
plt.savefig("atlas_susy_2018_04_Stau_cm3.png")
plt.show()
