...
# topos = [ "TGQ" ]
constraints = { "T2": "[[[q]],[[q]]]", "T1": "[[[q,q]],[[q,q]]]", "T6WW": "[[['q'],['W']],[['q'],['W']]]", "T6WWoff": "2.23 * [[['jet'],['jet','jet']],[['jet'],['jet','jet']]]", "T5WW": "[[['q','q'],['W']],[['q','q'],['W']]]", "T5ZZ": "[[['q','q'],['Z']],[['q','q'],['Z']]]", "T5WZh": "[[[q,q],[Z]],[[q,q],[W]]]+[[[q,q],[higgs]],[[q,q],[W]]]", "T6WZh": "[[[q],[Z]],[[q],[W]]]+[[[q],[higgs]],[[q],[W]]]", "T5WWoff": "2.23*[[['jet','jet'],['jet','jet']],[['jet','jet'],['jet','jet']]]", "TGQ": "[[[q]],[[q,q]]]" }
figure = { "T2": "Fig. 76a", "T1": "Fig. 76b", "T6WW": ('Fig. 77a', 'Fig. 77b' ), "T5WW": ( 'Fig. 77c', 'Fig. 77d' ), "T5ZZ": ('Fig.79', None) , "T5WZh": ('Fig. 78b', None ), "T6WZh":  ('Fig. 78a', None ), "T5WWoff": ( 'Fig. 77c', 'Fig. 77d' ), "T6WWoff": ('Fig. 77a', 'Fig. 77b' ), "TGQ": ('Fig. 80a', 'Fig. 80b', 'Fig. 80c' ) }
...
dataBaseUrl = "'https://www.hepdata.net/record/ins1641270?version=5&table=X-section U.L. & best SR @@NUM@@  : Meff'"
...
for topo in topos:
    ...
    for dsname in dsnames:
        ...
        if type (mPlanes) == list:
            ...
            Tx_1.dataUrl = dataBaseUrl.replace("@@NUM@@",str(eIdx) )
            obsCont = 'orig/Exclusioncontour(exp.)%d:Meff.csv' % eIdx
            expCont = 'orig/Exclusioncontour(obs.)%d:Meff.csv' % eIdx
            ...
            Tx_1.setSources(dataLabels= ['expExclusion', 'obsExclusion', 'efficiencyMap'],
                ...
                dataFiles= [ obsCont, expCont, 'orig/%s.embaked' % topoon ],
                units = units,
        ...
        if type (mPlanes) == tuple:
            ...
            for ctr,mp in enumerate(mPlanes):
                ...
                Tx_m.dataUrl = dataBaseUrl.replace("@@NUM@@",str(eIdx) )
                obsCont = 'orig/Exclusioncontour(exp.)%d:Meff.csv' % eIdx
                expCont = 'orig/Exclusioncontour(obs.)%d:Meff.csv' % eIdx
                units = [ None, None ]
                if "60." in str(mp):
