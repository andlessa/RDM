...
path=os.getcwd()
databasepath=path[:path.find("8TeV")]
print "path=",databasepath

expidname=path[path.find("8TeV")+5:].replace("CMS/","").replace("ATLAS/","").replace("/validation","")

print "expid=",expidname

database = DataBase( databasepath )

#How to validate all plots for all Txnames in one ExpRes:
expRes = database.getExpResults(analysisIDs=[expidname],datasetIDs=[None])
