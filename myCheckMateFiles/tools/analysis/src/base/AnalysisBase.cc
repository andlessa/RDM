...
bool AnalysisBase::checkTauTag(Jet* candidate, std::string efficiency) {
    if (efficiency == "loose" && jetTauTags[candidate][0])
        ...
        return true;
    else if (efficiency == "looseNew" && jetTauTags[candidate][3])
            return true;
    else if (efficiency == "mediumNew" && jetTauTags[candidate][4])
        return true;
    else if (efficiency == "tightNew" && jetTauTags[candidate][5])
        return true;
    return false;
}
