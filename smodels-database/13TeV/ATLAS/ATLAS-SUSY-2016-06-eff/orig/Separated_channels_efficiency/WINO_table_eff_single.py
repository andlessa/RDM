...
df = pd.read_csv(efficiency_files+'MSSM_single_output.dat',sep='\s+')
...
plt.axis('off')
plt.axis('tight')
plt.title('Table of Efficiencies for MSSM Model \n single production channel', fontsize = 15)
...
fig.tight_layout()
plt.savefig('MSSM_efficiency_table_single_channel.pdf')
...
fig.tight_layout()
plt.savefig('MSSM_xslim_table_single_channel.pdf')
...
fig.tight_layout()
plt.savefig('MSSM_accepted_xs_points_table_single_channel.pdf')

with open('MSSM_efficiency_single_channel.tex', 'w') as f:
   f.write(table_eff.to_latex())

with open('MSSM_xslim_single_channel.tex', 'w') as f2:
