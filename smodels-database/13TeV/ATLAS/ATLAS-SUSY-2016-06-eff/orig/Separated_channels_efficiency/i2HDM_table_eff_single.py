...
df = pd.read_csv(efficiency_files+'i2HDM_single_output.dat',sep='\s+')
...
plt.axis('off')
plt.axis('tight')
plt.title('Table of Efficiencies for i2HDM Model \n single production channel', fontsize = 15)
...
##Label of axes
plt.figtext(0.1, 0.55, "Lifetime (ns)", fontsize = 12, rotation=90)
plt.figtext(0.38, 0.85, "Chargino Mass (GeV)", fontsize = 12)

fig.tight_layout()
plt.savefig('i2HDM_efficiency_table_single_channel.pdf')
...
fig.tight_layout()
plt.savefig('i2HDM_xslim_table_single_channel.pdf')
...
plt.axis('off')
plt.axis('tight')
plt.title('Accepted values for production \n cross section in i2HDM Model \n for single production channel', fontsize = 13)
...
fig.tight_layout()
plt.savefig('i2HDM_accepted_xs_points_table_single_channel.pdf')

with open('i2HDM_efficiency_single_channel.tex', 'w') as f:
   f.write(table_eff.to_latex())

with open('i2HDM_xslim_single_channel.tex', 'w') as f2:
