...
# %% Load data
offCurve = np.genfromtxt('./CMS_data/CMS-SUS-16-048_Figure_006.csv',
                        delimiter=',', names=['mst','deltam'])

# %% Get data from CheckMate results
resultFolder = '../validation_results/cms_sus_16_048'
slhaFolder = '../validation_slha/'
recastData = []
srRecast = []
for slhaFile in glob.glob(slhaFolder+'/T6bbWWoff*.slha'):
...
contours = getContour(recastData[:,0],recastData[:,0]-recastData[:,1],recastData[:,3],levels=[0.8,1.0,1.2])
...
plt.plot(offCurve['mst'],offCurve['deltam'],linewidth=4,
            color='black',label='CMS-SUS-16-048')
...
cb.set_label("r")
plt.legend()
plt.title(r'$\tilde{t} \tilde{t}, \tilde{t} \to b + \tilde{\chi}_1^+, \tilde{\chi}_1^+ \to W^{*} \tilde{\chi}_1^0$ (Best SR Exclusion)')
plt.savefig("cms_sus_16_048_T6bbWWoff.png")
plt.show()
